...
const CardInstructorDashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadInstructorData = async () => {
            if (currentUser.uid) {
                await updateUser(
                    currentUser.uid,
                    {'instructor.total_courses_created': totalCourses} as Partial<User>
                );
            }
        };
        loadInstructorData();
    ...
    return (
        <div>
            <p className='font-abhaya text-2xl font-bold text-primary my-6'>
            ...
            {isDeleteModalOpen &&
                <DeleteCourseModal
                    ...
                    courseTitle={
                        allCourses.find((course) => course.course_id === selectedCourseId)
                            ?.course_title || ""
                    }
